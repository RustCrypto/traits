...
/// Sign the provided message bytestring using `Self` (e.g. a cryptographic key), returning a
/// digital signature on success, or the provided error type on failure.
///
/// This is a fallible equivalent of the [`Signer`] trait.
...
pub trait TryMultipartSigner<S> {
    ...
    /// Equivalent of [`TrySigner::try_sign()`] but the message is provided in non-contiguous byte
    /// slices.
    ///
    /// # Errors
    /// Returns implementation-specific errors in the event signing failed (e.g. KMS or HSM
...
impl<K, S> TryMultipartSigner<S> for K
where
    K: MultipartSigner<S>,
{
    type Error = Infallible;
...
pub trait SignerMut<S>: TrySignerMut<S, Error = Infallible> {
    /// Sign the given message, update the state, and return a digital signature.
    fn sign(&mut self, msg: &[u8]) -> S;
}

/// Sign the provided message bytestring using `&mut Self` (e.g. an evolving cryptographic key such
/// as a stateful hash-based signature), returning a digital signature on success, or the provided
...
/// Sign the given prehashed message `Digest` using `Self`.
///
/// This is a fallible equivalent of the [`DigestSigner`] trait.
#[cfg(feature = "digest")]
...
/// This is a fallible equivalent of the [`RandomizedSigner`] trait.
#[cfg(feature = "rand_core")]
...
/// This is a fallible equivalent of the [`RandomizedMultipartSigner`] trait.
#[cfg(feature = "rand_core")]
pub trait TryRandomizedMultipartSigner<S> {
    /// Error type.
    type Error: core::error::Error + Into<Error>;

    /// Equivalent of [`TryRandomizedSigner::try_sign_with_rng`] but the message is provided in
    /// non-contiguous byte slices.
    ///
    /// # Errors
    /// Returns implementation-specific errors in the event signing failed (e.g. KMS or HSM
...
/// This is a fallible equivalent of the [`RandomizedDigestSigner`] trait.
#[cfg(all(feature = "digest", feature = "rand_core"))]
...
/// This is a fallible equivalent of the [`RandomizedSignerMut`] trait.
#[cfg(feature = "rand_core")]
...
/// This is a fallible equivalent of the [`RandomizedMultipartSignerMut`] trait.
#[cfg(feature = "rand_core")]
pub trait TryRandomizedMultipartSignerMut<S> {
    /// Error type.
    type Error: core::error::Error + Into<Error>;

    /// Equivalent of [`TryRandomizedSignerMut::try_sign_with_rng()`] but the message is provided in
    /// non-contiguous byte slices.
    ///
    /// # Errors
    /// Returns implementation-specific errors in the event signing failed (e.g. KMS or HSM
...
pub trait AsyncSigner<S>: TryAsyncSigner<S, Error = Infallible> {
    /// Sign the given message, returning a digital signature.
    async fn sign_async(&self, msg: &[u8]) -> S;
}

/// This is a fallible equivalent of the [`AsyncSigner`] trait.
...
/// This is a fallible equivalent of the [`AsyncRandomizedSigner`] trait.
#[cfg(feature = "rand_core")]
