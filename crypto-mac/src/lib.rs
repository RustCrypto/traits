...
pub use crypto_common::{
    FixedOutput, FixedOutputReset, InvalidLength, Key, KeyInit, Reset, Update,
};
...
/// Tag bytes generated by a MAC algorithm `M`.
pub type TagBytes<M> = GenericArray<u8, <M as FixedOutput>::OutputSize>;
...
pub trait Mac: KeyInit + Update + FixedOutput {
    ...
    ///
    /// Returns `MacError` if `tag` is empty.
    ...
    /// is correct for the processed input.
    ///
    /// Returns `MacError` if `tag` is not valid or empty.
...
impl<M: Mac> Output<M> {
    ...
    pub fn new(bytes: TagBytes<M>) -> Output<M> {
        Output { bytes }
    }
    ...
    /// [`Mac`] trait.
    pub fn into_bytes(self) -> TagBytes<M> {
        self.bytes
    }
}
